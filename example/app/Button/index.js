...
var im = _interopRequireWildcard(_index);
...
console.log('proxyFile: ', '_index.js');
Object.keys(im).forEach(function (item) {
  if (Object.getPrototypeOf(im[item]).name === 'ReactComponent') {
    im2[item] = new _dist.ReactProxy(im[item], __dirname + '/_index.js');
  } else {
    im2[item] = new _dist.DefaultProxy({ default: im[item] }, item, __dirname + '/_index.js');
